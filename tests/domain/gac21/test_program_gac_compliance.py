...
VALID_PROGRAM_TYPE = "DSO_CPO_INTERFACE-2.1.0"
...
def test_program_gac_compliant_valid() -> None:
    ...
    assert program.program_type == VALID_PROGRAM_TYPE
...
def test_invalid_program_type_format() -> None:
    ...
    with pytest.raises(
        ValidationError,
        match=re.escape("The program type must equal DSO_CPO_INTERFACE-2.1.0."),
    ):
...
def test_program_multiple_errors_grouped() -> None:
    ...
    assert grouped_errors[0].get("msg", None) == "The program type must equal DSO_CPO_INTERFACE-2.1.0."
...
def test_plugin_system_integration() -> None:
    ...
    assert errors[1].get("msg", None) == "The program type must equal DSO_CPO_INTERFACE-2.1.0."
...
def test_plugin_with_edge_cases() -> None:
    ...
    test_cases = [
        ...
        (None, VALID_PROGRAM_TYPE, False, 2),  # retailer_name and binding_events fail
        (None, "DSO_CPO_INTERFACE-1.0.0", False, 3),  # retailer_name, program_type, binding_events fail
        ("X", VALID_PROGRAM_TYPE, True, 1),  # Only retailer_name too short
        ("X", "DSO_CPO_INTERFACE-1.0.0", True, 2),  # retailer_name too short and wrong program type
        ("Valid", "INVALID", True, 1),  # Only program_type invalid
        ("Valid", "DSO_CPO_INTERFACE-1.0.0", False, 2),  # program_type invalid and binding_events false
    ]
