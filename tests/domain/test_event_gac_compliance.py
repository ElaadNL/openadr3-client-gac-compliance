...
def _create_event(
        ...
        intervals: Tuple[Interval[EventPayload], ...] | None = None) -> NewEvent:
...
def test_missing_power_service_locations() -> None:
    ...
    with pytest.raises(ValueError, match="The event must contain a POWER_SERVICE_LOCATIONS target."):
        _ = _create_event(
...
def test_missing_ven_name() -> None:
    ...
    power_service_locations_target_only = (Target(type="POWER_SERVICE_LOCATIONS", values=("123456789012345678",)),)

    with pytest.raises(ValueError, match="The event must contain a VEN_NAME target."):
        _ = _create_event(
...
def test_multiple_power_service_location_targets() -> None:
    ...
    gac_required_targets = _default_valid_targets()
    additional_target = (Target(type="POWER_SERVICE_LOCATIONS", values=("test-target",)),)

    with pytest.raises(ValueError, match="The event must contain exactly one POWER_SERVICE_LOCATIONS target."):
        _ = _create_event(
            targets=gac_required_targets + additional_target,
            interval_period=None,
            intervals=(
...
def test_multiple_ven_name_targets() -> None:
    ...
    with pytest.raises(ValueError, match="The event must contain only one VEN_NAME target."):
        _ = _create_event(
            targets=gac_required_targets + additional_target,
            interval_period=None,
            intervals=(
...
def test_power_service_locations_target_value_empty() -> None:
    ...
    with pytest.raises(ValueError, match="The POWER_SERVICE_LOCATIONS target value cannot be empty."):
        _ = _create_event(
            targets=targets,
            interval_period=None,
            intervals=(
...
def test_power_service_locations_invalid_value_format() -> None:
    ...
    with pytest.raises(ValueError, match="The POWER_SERVICE_LOCATIONS target value must be a list of 'EAN18' values."):
        _ = _create_event(
            targets=targets,
            interval_period=None,
            intervals=(
...
def test_ven_name_target_value_empty() -> None:
    ...
    with pytest.raises(ValueError, match="The VEN_NAME target value cannot be empty."):
        _ = _create_event(
            targets=targets,
            interval_period=None,
            intervals=(
...
def test_ven_name_target_too_long() -> None:
    ...
    with pytest.raises(ValueError, match="The VEN_NAME target value must be a list of 'ven object name' values"):
        _ = _create_event(
            targets=targets,
            interval_period=None,
            intervals=(
...
def test_multiple_payload_descriptors() -> None:
    ...
    with pytest.raises(ValueError, match="The event must have exactly one payload descriptor."):
        _ = _create_event(
            payload_descriptor=(
                EventPayloadDescriptor(
                    payload_type=EventPayloadType.IMPORT_CAPACITY_LIMIT,
...
def test_invalid_payload_type_in_descriptor() -> None:
    ...
    with pytest.raises(ValueError, match="The payload descriptor must have a payload type of 'IMPORT_CAPACITY_LIMIT'."):
        _ = _create_event(
            payload_descriptor=(
                EventPayloadDescriptor(
                    payload_type=EventPayloadType.SIMPLE,
...
def test_invalid_unit_in_descriptor() -> None:
    ...
    with pytest.raises(ValueError, match="The payload descriptor must have a units of 'KW'"):
        _ = _create_event(
            payload_descriptor=(
                EventPayloadDescriptor(
                    payload_type=EventPayloadType.IMPORT_CAPACITY_LIMIT,
...
def test_priority_too_high() -> None:
    ...
    with pytest.raises(ValueError, match="The priority must be greater than or equal to 0 and less than or equal to 999."):
        _ = _create_event(
            priority=1000,
            interval_period=None,
            intervals=(
...
def test_priority_not_set() -> None:
    ...
    with pytest.raises(ValueError, match="The event must have a priority set."):
        _ = _create_event(
            priority=None,
            interval_period=None,
            intervals=(
...
def test_non_increasing_interval_ids() -> None:
    ...
    with pytest.raises(ValueError, match="The event interval must have an id value that is strictly increasing."):
        _ = _create_event(
            interval_period=None,
            intervals=(
                Interval(
...
def test_interval_payload_type_invalid() -> None:
    ...
    with pytest.raises(ValueError, match="The event interval payload must have a payload type of 'IMPORT_CAPACITY_LIMIT'."):
        _ = _create_event(
            interval_period=None,
            intervals=(
                Interval(
...
def test_event_interval_no_payload() -> None:
    ...
    with pytest.raises(ValueError, match="interval payload must contain at least one payload"):
        _ = _create_event(
            interval_period=IntervalPeriod(
                start=datetime(2023, 1, 1, 0, 0, 0, tzinfo=timezone.utc),
                duration=timedelta(minutes=5),
...
def test_event_interval_multiple_payloads() -> None:
    ...
    with pytest.raises(ValueError, match="The event interval must have exactly one payload."):
        _ = _create_event(
            interval_period=None,
            intervals=(
                Interval(
